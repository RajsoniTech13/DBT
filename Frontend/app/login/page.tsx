...
import { Eye, EyeOff, User, Shield, ArrowRight, ClipboardList, MapPin, SearchCode, Loader2 } from "lucide-react";
    ...
    <div className="min-h-screen w-full bg-[#F3F5F7]">
      ...
      <nav className="mx-auto flex w-full max-w-[1180px] items-center justify-between px-4 py-6 md:px-0">
        ...
        <div className="hidden gap-2 md:flex">
          <SecondaryButton onClick={() => window.location.href = '/signup'}>Sign Up</SecondaryButton>
        </div>
      </nav>
        ...
        <div className="flex flex-col justify-center space-y-8 pr-2">
          <div>
            ...
            <p className="mt-4 max-w-md text-slate-600">
              Sign in to access your DBT Dashboard and continue monitoring welfare leakage across Gujarat.
            </p>
          </div>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-lg ring-1 ring-slate-200">
              ...
              <AnimatePresence mode="popLayout" initial={false}>
                ...
                >
                  <div className="mt-6 mb-3 flex items-center gap-2">
                    {role === "dfo" && (
                      <><ClipboardList className="h-5 w-5 text-emerald-600" /><span className="text-lg font-semibold text-slate-900">DFO Login</span></>
                    )}
                    {role === "verifier" && (
                      <><MapPin className="h-5 w-5 text-emerald-600" /><span className="text-lg font-semibold text-slate-900">Verifier Login</span></>
                    )}
                    {role === "audit" && (
                      <><SearchCode className="h-5 w-5 text-emerald-600" /><span className="text-lg font-semibold text-slate-900">Audit Login</span></>
                    )}
                    {role === "admin" && (
                      <><Shield className="h-5 w-5 text-emerald-600" /><span className="text-lg font-semibold text-slate-900">Admin Login</span></>
                    )}
                  </div>

                  <p className="text-sm text-slate-500 mb-6 min-h-[40px]">
                    {role === "dfo" && "Investigate high-risk transactions and assign cases to field staff."}
                    {role === "verifier" && "Manage physical visits to beneficiaries and submit GPS-verified reports."}
                    {role === "audit" && "Query cross-scheme database flags and generate compliance summaries."}
                    {role === "admin" && "Configure system-wide leakage rules and view the state risk heatmap."}
                  </p>
                </motion.div>
              </AnimatePresence>
            </div>
            ...
            <form className="space-y-6" onSubmit={handleSignIn}>
              ...
              <div className="flex items-center justify-between text-sm">
                <CustomCheckbox name="rememberMe" label="Keep me signed in" />
                <a
                  href="#"
                  onClick={(e) => {
                    e.preventDefault();
              ...
              {error && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-3 text-sm text-red-600 bg-red-50 rounded-xl border border-red-100 mb-4">
                  {error}
                </motion.div>
              )}
              ...
              <PrimaryButton 
                type="submit" 
                className="w-full py-3.5 text-base flex items-center justify-center gap-2 group"
                disabled={isLoading}
              >
                {isLoading ? (
